...
export default function About({ innerRef }) {
    ...
    function formationText() {
        return <>
            <p><span style={{ color: info.baseColor }}>{firstName}{info.lastName.toLowerCase()} $</span> cd formation</p>
            <p><span style={{ color: info.baseColor }}>formation <span className={Style.green}>(main)</span> $</span> ls</p>
            <ul>
                {info.formation.map((item, index) => (
                    <li key={index}>
                        <Box component={'span'} mr={'1rem'}>{item.date}</Box>
                        <Box component={'span'} mr={'1rem'}>{item.institution}</Box>
    ...
    function experienceText() {
        return <>
            <p><span style={{ color: info.baseColor }}>{firstName}{info.lastName.toLowerCase()} $</span> cd experience</p>
            <p><span style={{ color: info.baseColor }}>experience <span className={Style.green}>(main)</span> $</span> ls</p>
            <ul>
                {info.experience.map((item, index) => (
                    <li key={index}>
                        <Box component={'span'} mr={'1rem'}>{item.date}</Box>
                        <Box component={'span'} mr={'1rem'}>{item.company}</Box>
                        <Box component={'span'}>{item.description}</Box>
                    </li>
                ))}
            </ul>
